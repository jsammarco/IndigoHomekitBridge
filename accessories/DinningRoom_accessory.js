...
var LightController = {
  name: "Dinning Room Lights", //name of accessory
  pincode: "031-45-155",
  ...
  serialNumber: "I Love Martiza", //serial number (optional)
  ...
  commandLights: function(light, opt) {
    var script = "";
    if (opt.brightness) {
      script = 'tell application "IndigoServer2" to brighten "'+this.name+'" to '+opt.brightness;
    } else {
      script = 'tell application "IndigoServer2" to turn '+(opt.status ? "on" : "off")+' "'+this.name+'"';
    }
    clearTimeout(this.timer);
    this.timer = setTimeout(function() {
      console.log("Script", script);
      applescript.execString(script, function(err, rtn) {
        if (err) { console.log("AppleScript Error: "+err); }
        if (rtn) { console.log(rtn); }
      });
    }, 200);
  },
  ...
  setPower: function(status) { //set power of accessory
    ...
    this.power = status;
    this.commandLights(this, {"status": status});
  },
  ...
  setBrightness: function(brightness) { //set brightness
    ...
    this.commandLights(this, {"brightness": brightness});
  },
